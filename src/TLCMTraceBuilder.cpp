...
TLCMTraceBuilder::TLCMTraceBuilder(const Configuration &conf)
...
TLCMTraceBuilder::~TLCMTraceBuilder(){
}

bool TLCMTraceBuilder::schedule(int *proc, int *aux, int *alt, bool *dryrun){
  if (cancelled) {
  ...
  *dryrun = false;
  *alt = 0;
  *aux = -1; /* No auxilliary threads in TLCM */
  if(replay){
...
void TLCMTraceBuilder::refuse_schedule(){
...
void TLCMTraceBuilder::mark_available(int proc, int aux){
  threads[ipid(proc,aux)].available = true;
}

void TLCMTraceBuilder::mark_unavailable(int proc, int aux){
  threads[ipid(proc,aux)].available = false;
}

bool TLCMTraceBuilder::is_replaying() const {
  return prefix_idx < replay_point;
}

void TLCMTraceBuilder::cancel_replay(){
  assert(replay == is_replaying());
  cancelled = true;
  replay = false;
}

void TLCMTraceBuilder::metadata(const llvm::MDNode *md){
  if(curev().md == 0){
    curev().md = md;
  }
  last_md = md;
...
bool TLCMTraceBuilder::sleepset_is_empty() const{
  return true;
}

Trace *TLCMTraceBuilder::get_trace() const{
...
void TLCMTraceBuilder::append_replay_events(std::vector<CMSEvent> &events_before, std::vector<Event> &new_prefix){
  // earlier events for replay 
  for( int i =0 ; i < events_before.size() ; i++){
    const CMSEvent &se = events_before[i];
...
bool TLCMTraceBuilder::reset(){
  ...
  if(!ccvschedules.scheduler.top()->new_read_from.empty() || !ccvschedules.scheduler.top()->new_schedules->scheduled_events.empty()) {
    ...
    //*****Create Schedules for postponed writes
      ...
      int read_tid;
      std::vector<CMSEvent> &schedule = ccvschedules.scheduler.top()->new_schedules->scheduled_rwevents.back();
      ...
      ccvschedules.scheduler.top()->read_from = read_tid - 1; // RWEvent index in vector events
      {
        ...
        std::vector<CMSEvent> &scheduled_events = ccvschedules.scheduler.top()->new_schedules->scheduled_events.back();
        for( int i =0 ; i < scheduled_events.size() ; i++) {
          CMSEvent &se = scheduled_events[i];
          int pid = se.iid.get_pid();
          int index = se.iid.get_index();
          IID<IPid> iid(pid, index);
          new_prefix.emplace_back(iid);
...
IID<CPid> TLCMTraceBuilder::get_iid() const{
  IPid pid = curev().iid.get_pid();
...
std::string TLCMTraceBuilder::iid_string(std::size_t pos) const{
  return iid_string(prefix[pos]);
}

std::string TLCMTraceBuilder::iid_string(const Event &event) const{
...
std::string TLCMTraceBuilder::iid_string(IID<IPid> iid) const{
  return iid_string(find_process_event(iid.get_pid(), iid.get_index()));
}

void TLCMTraceBuilder::debug_print() const {
  llvm::dbgs() << "TLCMTraceBuilder (debug print, replay until " << replay_point << "):\n";
  int idx_offs = 0;
  int iid_offs = 0;
  int rf_offs = 0;
  int clock_offs = 0;
...
bool TLCMTraceBuilder::store(const SymData &sd){
  assert(false && "Cannot happen");
  abort();
  return true;
}

bool TLCMTraceBuilder::atomic_store(const SymData &sd){
...
void TLCMTraceBuilder::do_atomic_store(const SymData &sd){
...
bool TLCMTraceBuilder::atomic_rmw(const SymData &sd ,  RmwAction action){
  return true;
}

bool TLCMTraceBuilder::load(const SymAddrSize &ml){
...
void TLCMTraceBuilder::do_load(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool TLCMTraceBuilder::compare_exchange
...
bool TLCMTraceBuilder::full_memory_conflict(){
  invalid_input_error("TLCM does not support black-box functions with memory effects");
  return false;
...
bool TLCMTraceBuilder::fence(){
  return true;
}

bool TLCMTraceBuilder::join(int tgt_proc){
...
void TLCMTraceBuilder::add_happens_after(unsigned second, unsigned first){
  assert(first != ~0u);
  assert(second != ~0u);
  assert(first != second);
  assert(first < second);
...
void TLCMTraceBuilder::add_happens_after_thread(unsigned second, IPid thread){
...
bool TLCMTraceBuilder::record_symbolic(SymEv event){
  if (!replay) {
    assert(!seen_effect);
    /* New event */
...
bool TLCMTraceBuilder::is_load(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  return e.kind == SymEv::LOAD;
}

bool TLCMTraceBuilder::is_minit(unsigned i) const {
  return prefix[i].sym.kind == SymEv::M_INIT;
}

bool TLCMTraceBuilder::is_mdelete(unsigned i) const {
  return prefix[i].sym.kind == SymEv::M_DELETE;
}


bool TLCMTraceBuilder::is_store(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  return e.kind == SymEv::STORE;
}


bool TLCMTraceBuilder::is_store_when_reading_from(unsigned i, int read_from) const {
  const SymEv &e = prefix[i].sym;
...
SymAddrSize TLCMTraceBuilder::get_addr(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  if (e.has_addr()) {
    return e.addr();
  }
...
SymData TLCMTraceBuilder::get_data(int i, const SymAddrSize &addr) const {
  if (i == -1) {
...
bool TLCMTraceBuilder::happens_before(const RWEvent &e,
...
std::vector<int> TLCMTraceBuilder::iid_map_at(int event) const{
...
void TLCMTraceBuilder::iid_map_step(std::vector<int> &iid_map, const Event &event) const{
  if (iid_map.size() <= unsigned(event.iid.get_pid())) iid_map.resize(event.iid.get_pid()+1, 1);
  iid_map[event.iid.get_pid()] += event.size;
}

void TLCMTraceBuilder::iid_map_step_rev(std::vector<int> &iid_map, const Event &event) const{
  iid_map[event.iid.get_pid()] -= event.size;
}

inline Option<unsigned> TLCMTraceBuilder::try_find_process_event(IPid pid, int index) const{
...
inline unsigned TLCMTraceBuilder::find_process_event(IPid pid, int index) const{
  assert(pid >= 0 && pid < int(threads.size()));
...
long double TLCMTraceBuilder::estimate_trace_count() const{
  return estimate_trace_count(0);
}

bool TLCMTraceBuilder::check_for_cycles() {
  return false;
}

long double TLCMTraceBuilder::estimate_trace_count(int idx) const{
...
/////////////////////////////////////////////////////////////....................................
void TLCMTraceBuilder::beginEvent(int tid){
  assert(tid > 0);
  IPid pid = curev().iid.get_pid();
  ++event_idx;
...
int TLCMTraceBuilder::compute_above_clock(unsigned i) {
  int last = -1;
  ...
  if (tidx > 1) {
    ...
    events[i].pi_clocks = events[last].pi_clocks;
    /*for(auto pi:events[last].pi_clocks){
      events[i].pi_clocks[pi.first] = events[last].pi_clocks[pi.first];
    }*/
    events[i].above_clock = events[last].above_clock;
  } else {
    //events[i].clock = VClock<IPid>();
    for(int p = 0 ; p < threads.size() ; p++){
      events[i].pi_clocks[p] = VClock<IPid>(); 
    }
    events[i].above_clock = VClock<IPid>();
  }
  //events[i].clock[ipid] = tidx;
  for(auto p:events[i].pi_clocks){
    events[i].pi_clocks[p.first][ipid] = tidx;
  }
...
void TLCMTraceBuilder::compute_vclocks(){
  ...
  if(events[i].eventType.kind ==SymEv::LOAD && events[i].read_from != -1) {
        events[i].pi_clocks[events[i].pid] += events[events[i].read_from].pi_clocks[events[i].pid];
        events[i].above_clock += events[events[i].read_from].above_clock;
      }// [po U rf ]* 

  //Saturation steps:  Add pi_shb edges
  if(!events[i].modification_order.empty()){
      for(auto p : events[i].modification_order){
          for(auto j: events[i].modification_order[p.first]){
            if (j != -1) {
              events[i].pi_clocks[p.first] += events[j].pi_clocks[p.first];
            }
          }
      }
    }
...
bool TLCMTraceBuilder::has_store_on_var(const void *ptr, unsigned i) const {
...
int TLCMTraceBuilder:: performWrite(void *ptr, llvm::GenericValue val , int typ){
  if(!typ)
    return -1;
  curev().may_conflict = true;
...
void TLCMTraceBuilder::record_replay(int eindex , int teindex){

    std::vector<CMSEvent> &events_before = prefix[eindex].new_schedules->events_before;
...
int TLCMTraceBuilder::performRead(void *ptr , int typ) {
  if(!typ)
    return -1;
  ...
  if(replay){
    ...
    if(read_from!= -1) {
      ...
      //rf edge
      // make rf visible in this proc clock
      events[event_idx].pi_clocks[events[event_idx].pid] += events[read_from].pi_clocks[events[event_idx].pid]; 
      events[event_idx].above_clock += events[read_from].above_clock;
      //co edges
      ...
      //Add co edge to events[reads_from] form we \in [po U rf] 
      for(auto we : visible_vec){
        if(we != read_from){
            events[read_from].modification_order[events[event_idx].pid].insert(we);
            events[read_from].pi_clocks[events[event_idx].pid] += events[we].pi_clocks[events[event_idx].pid]; // make co edge visible in vector clock
        }
      }
    //Saturation step 2
    for(int e=event_idx ; e >= 0 ; e--){
      if(events[e].pid == events[event_idx].pid && events[e].eventType.kind == SymEv::LOAD){
        for(int ej = event_idx-1 ; ej >=0 ; ej--){
          if(e!=ej && !events[ej].global_variables.empty() && events[ej].global_variables.count(events[e].readVar) != 0){
            if(events[e].read_from == -1 || ( events[e].read_from != ej &&
                happens_before(events[events[e].read_from],events[ej].pi_clocks[events[e].pid]))){
              events[ej].pi_clocks[events[e].pid] += events[e].pi_clocks[events[e].pid];
              if(events[ej].modification_order[events[e].pid].count(e) == 0){
                  events[ej].modification_order[events[e].pid].insert(e);
              }
            }
          }
        }
  ...
  //*** Return -1, if no event has a write on this variable, i.e read from init
  {
    ...
    if(!flag){
      ...
      std::shared_ptr<CMSchedule> pt(new CMSchedule(prefix_idx));
  ...
  for(auto j : hb_before){
    int readable = true;
    for( auto k : hb_before){
      if(j!= k && happens_before(events[j] , events[k].pi_clocks[events[event_idx].pid])){
        readable = false;
      }
    }
    if(readable){
  ...
  if(!curev().can_read_from.empty()) {
    ...
    if(reads_from != -1) {
      // make rf visible in this proc clock
      events[event_idx].pi_clocks[events[event_idx].pid] += events[reads_from].pi_clocks[events[event_idx].pid]; 
    ...
    if( reads_from != -1){        //Ensures that it is not init
      for(auto we : visible_vec){
        if(we != reads_from){
            events[reads_from].modification_order[events[event_idx].pid].insert(we);
            events[reads_from].pi_clocks[events[event_idx].pid] += events[we].pi_clocks[events[event_idx].pid]; // make co edge visible in vector clock
        }
      }
    }
...
void TLCMTraceBuilder::createSchedule(int tid){
  IPid pid = curev().iid.get_pid();
  RWEvent &e = events[event_idx];
      ...
      for(int i = event_idx-1 ; i >= 0 ; i--){
          ...
          if(events[i].eventType.kind == SymEv::LOAD && events[i].readVar == ptr.first && 
            ...
            if(possible){
              ...
              std::vector<CMSEvent> rwevents_schedule;
              std::vector<CMSEvent> events_schedule;

              int cur_tid = events[i].tid-1;
               //*** Create schedule starting from write event etid.
              for( int j = i+1 ; j <= event_idx; j++){
                if(happens_before(events[j],e.above_clock)){
                  CMSEvent rwe(events[j].get_pid(), ++cur_tid , events[j].get_index());
...
bool TLCMTraceBuilder::is_equivalent(std::vector<std::vector<CMSEvent>> &schedules , std::vector<CMSEvent> this_schedule) {
  if(schedules.empty())
    return false;
  bool res = false;
  for(int i=0 ; i < schedules.size() ; i++){
    std::vector<CMSEvent> &schedule = schedules[i];
    int count = 0;
...
uint64_t TLCMTraceBuilder::tracecount(){
  uint64_t t = 0;
  int64_t value = 0;
  t = temp;
  //t = tp;
