...
#include <config.h>
#ifndef __TLCC_TRACE_BUILDER_H__
#define __TLCC_TRACE_BUILDER_H__
...
class TLCCTraceBuilder final : public TSOPSOTraceBuilder{
public:
  TLCCTraceBuilder(const Configuration &conf = Configuration::default_conf);
  virtual ~TLCCTraceBuilder() override;
...
protected:
  ...
  public:
    ...
    std::unordered_map<const void *, llvm::GenericValue> global_variables;
    std::map<const void * , std::set<int>> hb_writes;
    std::vector<Tid> modification_order; // incomein co //////////////////////////////////
  ...
  class RWEvent{
    ...
    int read_from; // index in transactions
  ...
  TLCCSchedules ccvschedules;
  std::unordered_map<int,std::shared_ptr<CCSEvent>> current_schedules; // depth->CCSEvent
  ...
  bool is_equivalent(std::vector<std::vector<CCSEvent>> &schedules , std::vector<CCSEvent> this_schedule);
  void add_to_queue();
  void append_replay_events(std::vector<CCSEvent> &events_before, std::vector<Event> &new_prefix);
