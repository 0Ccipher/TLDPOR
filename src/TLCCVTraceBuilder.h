...
#include <config.h>
#ifndef __TLCCV_TRACE_BUILDER_H__
#define __TLCCV_TRACE_BUILDER_H__
...
class TLCCVTraceBuilder final : public TSOPSOTraceBuilder{
public:
  TLCCVTraceBuilder(const Configuration &conf = Configuration::default_conf);
  virtual ~TLCCVTraceBuilder() override;
...
protected:
  ...
  bool is_equivalent(std::vector<std::vector<SEvent>> &schedules , std::vector<SEvent> this_schedule);
  void add_to_queue();
  void append_replay_events(std::vector<SEvent> &events_before, std::vector<Event> &new_prefix);
