...
create_execution_engine(TraceBuilder &TB, llvm::Module *mod,
  ...
  case Configuration::ARM: case Configuration::POWER:
    ...
    break;
  case Configuration::TLCCV:
    EE = llvm::Interpreter::create(mod,static_cast<TSOPSOTraceBuilder&>(TB),conf,&ErrorMsg);
    break;
  case Configuration::TLCM:
    EE = llvm::Interpreter::create(mod,static_cast<TSOPSOTraceBuilder&>(TB),conf,&ErrorMsg);
    break;
  case Configuration::TLCC:
    EE = llvm::Interpreter::create(mod,static_cast<TSOPSOTraceBuilder&>(TB),conf,&ErrorMsg);
    break;
...
DPORDriver::Result DPORDriver::run_ccv() {
  Result res;
  auto context = std::make_unique<llvm::LLVMContext>();
  std::unique_ptr<llvm::Module> mod = parse(PARSE_AND_CHECK,
                                            *context);
  TLCCVTraceBuilder TB(conf);
...
DPORDriver::Result DPORDriver::run_cm() {
  Result res;
  auto context = std::make_unique<llvm::LLVMContext>();
  std::unique_ptr<llvm::Module> mod = parse(PARSE_AND_CHECK,
                                            *context);
  TLCMTraceBuilder TB(conf);
