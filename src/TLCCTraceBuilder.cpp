...
TLCCTraceBuilder::TLCCTraceBuilder(const Configuration &conf)
...
TLCCTraceBuilder::~TLCCTraceBuilder(){
}

bool TLCCTraceBuilder::schedule(int *proc, int *aux, int *alt, bool *dryrun){
  if (cancelled) {
  ...
  *dryrun = false;
  *alt = 0;
  *aux = -1; /* No auxilliary threads in TLCC */
  if(replay){
...
void TLCCTraceBuilder::refuse_schedule(){
...
void TLCCTraceBuilder::mark_available(int proc, int aux){
  threads[ipid(proc,aux)].available = true;
}

void TLCCTraceBuilder::mark_unavailable(int proc, int aux){
  threads[ipid(proc,aux)].available = false;
}

bool TLCCTraceBuilder::is_replaying() const {
  return prefix_idx < replay_point;
}

void TLCCTraceBuilder::cancel_replay(){
  assert(replay == is_replaying());
  cancelled = true;
  replay = false;
}

void TLCCTraceBuilder::metadata(const llvm::MDNode *md){
  if(curev().md == 0){
    curev().md = md;
  }
  last_md = md;
...
bool TLCCTraceBuilder::sleepset_is_empty() const{
  return true;
}

Trace *TLCCTraceBuilder::get_trace() const{
...
void TLCCTraceBuilder::append_replay_events(std::vector<CCSEvent> &events_before, std::vector<Event> &new_prefix){
...
bool TLCCTraceBuilder::reset(){
  ...
  if(!ccvschedules.scheduler.top()->new_read_from.empty() || !ccvschedules.scheduler.top()->new_schedules->scheduled_events.empty()) {
    ...
    //*****Create Schedules for postponed writes
      ...
      int read_tid;
      std::vector<CCSEvent> &schedule = ccvschedules.scheduler.top()->new_schedules->scheduled_rwevents.back();
      ...
      ccvschedules.scheduler.top()->read_from = read_tid - 1; // RWEvent index in vector events
      {
        ...
        std::vector<CCSEvent> &scheduled_events = ccvschedules.scheduler.top()->new_schedules->scheduled_events.back();
...
IID<CPid> TLCCTraceBuilder::get_iid() const{
  IPid pid = curev().iid.get_pid();
...
std::string TLCCTraceBuilder::iid_string(std::size_t pos) const{
  return iid_string(prefix[pos]);
}

std::string TLCCTraceBuilder::iid_string(const Event &event) const{
...
std::string TLCCTraceBuilder::iid_string(IID<IPid> iid) const{
  return iid_string(find_process_event(iid.get_pid(), iid.get_index()));
}

void TLCCTraceBuilder::debug_print() const {
  llvm::dbgs() << "TLCCTraceBuilder (debug print, replay until " << replay_point << "):\n";
  int idx_offs = 0;
  int iid_offs = 0;
  int rf_offs = 0;
  int clock_offs = 0;
...
bool TLCCTraceBuilder::store(const SymData &sd){
  assert(false && "Cannot happen");
  abort();
  return true;
}

bool TLCCTraceBuilder::atomic_store(const SymData &sd){
...
void TLCCTraceBuilder::do_atomic_store(const SymData &sd){
...
bool TLCCTraceBuilder::atomic_rmw(const SymData &sd ,  RmwAction action){
  return true;
}

bool TLCCTraceBuilder::load(const SymAddrSize &ml){
...
void TLCCTraceBuilder::do_load(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool TLCCTraceBuilder::compare_exchange
...
bool TLCCTraceBuilder::full_memory_conflict(){
  invalid_input_error("TLCC does not support black-box functions with memory effects");
  return false;
...
bool TLCCTraceBuilder::fence(){
  return true;
}

bool TLCCTraceBuilder::join(int tgt_proc){
...
void TLCCTraceBuilder::add_happens_after(unsigned second, unsigned first){
  assert(first != ~0u);
  assert(second != ~0u);
  assert(first != second);
  assert(first < second);
...
void TLCCTraceBuilder::add_happens_after_thread(unsigned second, IPid thread){
...
bool TLCCTraceBuilder::record_symbolic(SymEv event){
  if (!replay) {
    assert(!seen_effect);
    /* New event */
...
bool TLCCTraceBuilder::is_load(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  return e.kind == SymEv::LOAD;
}

bool TLCCTraceBuilder::is_minit(unsigned i) const {
  return prefix[i].sym.kind == SymEv::M_INIT;
}

bool TLCCTraceBuilder::is_mdelete(unsigned i) const {
  return prefix[i].sym.kind == SymEv::M_DELETE;
}


bool TLCCTraceBuilder::is_store(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  return e.kind == SymEv::STORE;
}


bool TLCCTraceBuilder::is_store_when_reading_from(unsigned i, int read_from) const {
  const SymEv &e = prefix[i].sym;
...
SymAddrSize TLCCTraceBuilder::get_addr(unsigned i) const {
  const SymEv &e = prefix[i].sym;
  if (e.has_addr()) {
    return e.addr();
  }
...
SymData TLCCTraceBuilder::get_data(int i, const SymAddrSize &addr) const {
  if (i == -1) {
...
bool TLCCTraceBuilder::happens_before(const RWEvent &e,
...
std::vector<int> TLCCTraceBuilder::iid_map_at(int event) const{
...
void TLCCTraceBuilder::iid_map_step(std::vector<int> &iid_map, const Event &event) const{
  if (iid_map.size() <= unsigned(event.iid.get_pid())) iid_map.resize(event.iid.get_pid()+1, 1);
  iid_map[event.iid.get_pid()] += event.size;
}

void TLCCTraceBuilder::iid_map_step_rev(std::vector<int> &iid_map, const Event &event) const{
  iid_map[event.iid.get_pid()] -= event.size;
}

inline Option<unsigned> TLCCTraceBuilder::try_find_process_event(IPid pid, int index) const{
...
inline unsigned TLCCTraceBuilder::find_process_event(IPid pid, int index) const{
  assert(pid >= 0 && pid < int(threads.size()));
...
long double TLCCTraceBuilder::estimate_trace_count() const{
  return estimate_trace_count(0);
}

bool TLCCTraceBuilder::check_for_cycles() {
  return false;
}

long double TLCCTraceBuilder::estimate_trace_count(int idx) const{
...
/////////////////////////////////////////////////////////////....................................
void TLCCTraceBuilder::beginEvent(int tid){
  assert(tid > 0);
  IPid pid = curev().iid.get_pid();
  ++event_idx;
...
int TLCCTraceBuilder::compute_above_clock(unsigned i) {
  int last = -1;
...
void TLCCTraceBuilder::compute_vclocks(){
...
bool TLCCTraceBuilder::has_store_on_var(const void *ptr, unsigned i) const {
...
int TLCCTraceBuilder:: performWrite(void *ptr, llvm::GenericValue val , int typ){
  if(!typ)
    return -1;
  curev().may_conflict = true;
...
void TLCCTraceBuilder::record_replay(int eindex , int teindex){

    std::vector<CCSEvent> &events_before = prefix[eindex].new_schedules->events_before;
...
int TLCCTraceBuilder::performRead(void *ptr , int typ) {
  if(!typ)
    return -1;
    ...
    if(read_from!= -1) {
      ...
      for(auto j : hb_before){
        ...
        for( auto k : hb_before){
          if(j!= k && happens_before(events[j] , events[k].above_clock)){
            readable = false;
          }
        }
        if(readable){
  ...
  if(!curev().can_read_from.empty()) {
    ...
    //Add rf edge
    if(reads_from != -1) {
       // make rf visible in vector clocks
...
void TLCCTraceBuilder::add_to_queue(){
...
void TLCCTraceBuilder::createSchedule(int tid){
  IPid pid = curev().iid.get_pid();
  RWEvent &e = events[event_idx];
      ...
      for(int i = event_idx-1 ; i >= 0 ; i--){
          ...
          if(events[i].eventType.kind == SymEv::LOAD && events[i].readVar == ptr.first && 
            ...
            if(possible){
              ...
              std::vector<CCSEvent> rwevents_schedule;
              ...
              for( int j = i+1 ; j <= event_idx; j++){
                if(happens_before(events[j],e.above_clock)){
                  CCSEvent rwe(events[j].get_pid(), ++cur_tid , events[j].get_index());
...
bool TLCCTraceBuilder::is_equivalent(std::vector<std::vector<CCSEvent>> &schedules , std::vector<CCSEvent> this_schedule) {
  if(schedules.empty())
    return false;
  bool res = false;
  for(int i=0 ; i < schedules.size() ; i++){
    std::vector<CCSEvent> &schedule = schedules[i];
    int count = 0;
...
uint64_t TLCCTraceBuilder::tracecount(){
  uint64_t t = 0;
  int64_t value = 0;
  t = temp;
  //t = tp;
