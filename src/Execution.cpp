...
void Interpreter::visitLoadInst(LoadInst &I) {
  ...
  {
    TB.beginEvent(++events);
    ++event_idx;
  	assert(event_idx == int(events_vec.size()));
  	Event e(CurrentThread,events);
    events_vec.emplace_back(e);
  }
...
void Interpreter::visitStoreInst(StoreInst &I) {
  ...
  //int tid = 1;
  if(tid > -1){
	  assert(!events_vec.empty());
		Event &t = events_vec.back();
		if(t.global_variables.count(Ptr)){
...
void Interpreter::run() {
  ...
  CurrentThread = 0;
  for(unsigned i=0 ; i < events_vec.size() ; ++i){
			events_vec[i].global_variables.clear();
	}
  events_vec.clear();
  events = 0;
  event_idx = -1;
